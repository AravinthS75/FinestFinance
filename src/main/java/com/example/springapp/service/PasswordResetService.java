...
public class PasswordResetService {
    ...
    private void sendPasswordResetEmail(User user, String token) {
        ...
        String content = "<!DOCTYPE html>"
            ...
            + "    }"
            + "    /* Fallback inline for old clients */"
            + "  </style>"
            + "</head>"
            + "<body>"
            + "  <p>💾 Hi <strong>" + user.getName() + "</strong>,</p>"
            + "  <p>We saw a request to reset your password in the matrix.</p>"
            + "  <p>Click the button below to upload your new creds:</p>"
            + "  <p>"
            + "    <a href=\"" + resetUrl + "\""
            + "       class=\"btn\""
            + "       style=\""
            ...
            + "    This link self-destructs in <strong>1 hour</strong>.<br>"
            + "    If you didn't ask, simply ignore this message."
            + "  </p>"
            + "  <p>— Finest Finance Team</p>"
            + "</body>"
            + "</html>";
